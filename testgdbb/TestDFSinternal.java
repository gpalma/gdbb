 ...
 */
public class TestDFSinternal extends Test {

  	String[] newV;
  	int N;

	public TestDFSinternal() {
		TestName = "Internal DFS";
	}

	protected boolean checkDFS() {
		boolean res = true;
		for (int i=0 ; i<N-2 && res ; i++) {
			if (this.graphTest.dfs(newV[i], newV[i+1])
				&& this.graphTest.dfs(newV[i+1], newV[i+2]))
				res = this.graphTest.dfs(newV[i], newV[i+2]);
		}
		return res;
	}
