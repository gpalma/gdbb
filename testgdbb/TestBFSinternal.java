 ...
 */
public class TestBFSinternal extends Test {

	String[] newV;
  	int N;

	public TestBFSinternal() {
		TestName = "Internal BFS";
	}

	protected boolean checkBFS() {
		boolean res = true;
		for (int i=0 ; i<N-2 && res ; i++) {
			if (this.graphTest.bfs(newV[i], newV[i+1])
				&& this.graphTest.bfs(newV[i+1], newV[i+2]))
				res = this.graphTest.bfs(newV[i], newV[i+2]);
		}
		return res;
	}
